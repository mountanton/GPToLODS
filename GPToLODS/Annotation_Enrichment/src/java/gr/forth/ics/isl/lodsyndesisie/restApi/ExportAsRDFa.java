...
@Path("/exportAsRDFa")
public class ExportAsRDFa extends ArrayList<String> {
        ...
        if(text==null){
          ...
          //   JSONObject job=new JSONObject();
           //  job.put("error","Unknown Query Parameter. Only the following query parameters are accepted: [uri,provenance (optional)]");
            return Response.status(status).type("text/html").entity("Error").build();
        }
        else{
            
            String output="";
            try {
                output = func.text2RDFa(text, tool);
 
            } catch (IOException ex) {
                Logger.getLogger(ExportAsRDFa.class.getName()).log(Level.SEVERE, null, ex);
            } catch (JSONException ex) {
                Logger.getLogger(ExportAsRDFa.class.getName()).log(Level.SEVERE, null, ex);
            }
           
                return Response.status(status).type("text/html").entity(output).build();
